...
import { QuotationType } from '../../../../../apps/isomorphic-i18n/src/validators/quotation-builder.schema';
                ...
                <tr>
                  <td>
                    ...
                    <div className="mt-12 grid grid-cols-2 gap-8">
                      ...
                      <div className="text-end">
                        <Text className="mb-1 font-medium text-gray-900">
                          {data.quotation.QuotationNumberLabel}
                        </Text>
                        <Text className="mb-5">{data.quotation.number}</Text>
                        <Text className="mb-1 font-medium text-gray-900">
                          {data.quotation.QuotationDateLabel}
                        </Text>
                        <Text>
                          {data.quotation.date?.toDateString() ?? '-- --'}
                        </Text>
                      </div>
                    </div>
                    ...
                    <div className="mt-12 w-full break-inside-auto border-b">
                      <div className="quotation-print-table-header !mb-2 grid break-inside-avoid grid-cols-12 gap-2 px-2 font-medium">
                        <span className="col-span-4 block py-2 text-start">
                          {data.quotationTableHeader.title}
                        </span>
                        <span className="col-span-2 block py-2 text-end">
                          {data.quotationTableHeader.quantity}
                        </span>
                        <span className="col-span-2 block py-2 text-end">
                          {data.quotationTableHeader.rate}
                        </span>
                        <span className="col-span-2 block py-2 text-end">
                          {data.quotationTableHeader.tax}
                        </span>
                        <span className="col-span-2 block py-2 text-end">
                          {data.quotationTableHeader.amount}
                        </span>
                      </div>
                      {data.quotationTable.map((item, index) => (
                        <div
                          key={index}
