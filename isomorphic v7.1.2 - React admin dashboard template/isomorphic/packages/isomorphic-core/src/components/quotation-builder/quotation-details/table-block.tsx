      ...
      <div className="grid grid-cols-12 gap-2 rounded-t-md bg-gray-100 p-2 dark:bg-gray-900">
        <TableHeaderCell className="col-span-4 ps-2">
          <QuotationInput
            ...
            {...register('quotationTableHeader.title')}
          />
        </TableHeaderCell>
        <TableHeaderCell className="col-span-2 justify-end">
          <QuotationInput
            ...
            {...register('quotationTableHeader.quantity')}
          />
        </TableHeaderCell>
        <TableHeaderCell className="col-span-2 justify-end">
          <QuotationInput
            placeholder="Rate"
            inputClassName="[&_input]:text-end"
            {...register('quotationTableHeader.rate')}
          />
        </TableHeaderCell>
        <TableHeaderCell className="col-span-2 justify-end">
          <QuotationInput
            placeholder="Tax"
            inputClassName="[&_input]:text-end"
            {...register('quotationTableHeader.tax')}
          />
        </TableHeaderCell>
        <TableHeaderCell className="col-span-2 justify-end">
          <QuotationInput
            placeholder="Amount"
            inputClassName="[&_input]:text-end"
            {...register('quotationTableHeader.amount')}
          />
        </TableHeaderCell>
      </div>
      <ul>
        <SortableList items={fields} onChange={handleChange}>
          {fields?.map((field, index) => {
            let q = getValues(`quotationTable.${index}.quantity`);
            let r = getValues(`quotationTable.${index}.rate`);
            let total = q * r;
            return (
              <Fragment key={`quotation-table-${index}`}>
                  ...
                  <div className="group grid min-h-10 grid-cols-12 gap-0 border-b border-muted dark:border-muted/20">
                    <div className="col-span-4 py-2 pe-2">
                      <QuotationTextarea
                        ...
                        {...register(`quotationTable.${index}.title`)}
                      />
                    </div>
                    <div className="col-span-2 p-2">
                      <QuotationInput
                        ...
                        placeholder="1"
                        inputClassName="[&_input]:text-end"
                        {...register(`quotationTable.${index}.quantity`, {
                          valueAsNumber: true,
                        })}
                      />
                    </div>
                    <div className="col-span-2 p-2">
                      <QuotationInput
                        ...
                        placeholder="120"
                        inputClassName="[&_input]:text-end"
                        {...register(`quotationTable.${index}.rate`, {
                          valueAsNumber: true,
                        })}
                      />
                    </div>
                    <div className="col-span-2 p-2">
                      <QuotationInput
                        ...
                        placeholder="5%"
                        inputClassName="[&_input]:text-end"
                        {...register(`quotationTable.${index}.tax`, {
                          valueAsNumber: true,
                        })}
                      />
                    </div>
