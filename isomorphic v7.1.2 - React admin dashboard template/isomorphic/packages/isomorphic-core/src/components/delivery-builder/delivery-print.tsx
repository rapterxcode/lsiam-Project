...
import { DeliveryType } from '../../../../../apps/isomorphic-i18n/src/validators/delivery-builder.schema';
...
export const DeliveryPrint = forwardRef<HTMLDivElement, DeliveryPrintProps>(
                    ...
                    <div className="mt-12 grid grid-cols-2 gap-8">
                      ...
                      <div className="text-end">
                        <Text className="mb-1 font-medium text-gray-900">
                          {data.delivery.DeliveryNumberLabel}
                        </Text>
                        <Text className="mb-5">{data.delivery.number}</Text>
                        <Text className="mb-1 font-medium text-gray-900">
                          {data.delivery.DeliveryDateLabel}
                        </Text>
                        <Text>
                          {data.delivery.date?.toDateString() ?? '-- --'}
                        </Text>
                      </div>
                    </div>
                    ...
                    <div className="mt-12 w-full break-inside-auto border-b">
                      <div className="invoice-print-table-header !mb-2 grid break-inside-avoid grid-cols-12 gap-2 px-2 font-medium">
                        <span className="col-span-4 block py-2 text-start">
                          {data.deliveryTableHeader.title}
                        </span>
                        <span className="col-span-2 block py-2 text-end">
                          {data.deliveryTableHeader.quantity}
                        </span>
                        <span className="col-span-2 block py-2 text-end">
                          {data.deliveryTableHeader.rate}
                        </span>
                        <span className="col-span-2 block py-2 text-end">
                          {data.deliveryTableHeader.tax}
                        </span>
                        <span className="col-span-2 block py-2 text-end">
                          {data.deliveryTableHeader.amount}
                        </span>
                      </div>
                      {data.deliveryTable.map((item, index) => (
                        <div
                          key={index}
...
DeliveryPrint.displayName = 'DeliveryPrint';
